...
class DrillProcess():
    ...
    def get_diameter_list(isEnDiameter):
        if isEnDiameter:
            ...
            diameter_list = DrillProcess.get_drill_diameter_list(100000, 6500000, 50000, [])    #单位：纳米  
        return diameter_list

    def drill_resize_to(self, job, step, data):
        time.sleep(0.5)
        ##################传参#########################
        ...
        #获取刀径表  
        diameter_list = DrillProcess.get_diameter_list(isEnDiameter)
        # if isEnDiameter:
        #     diameter_list = [3.9, 5.9, 7.9, 9.8, 11.8, 13.8, 15.7, 17.7, 19.7, 21.7, 23.6, 25.6, 27.6, 29.5, 31.5, 33.5, 35.4, 37.4, 
        #                     39.4, 41.3, 43.3, 45.3, 47.2, 49.2, 51.2, 53.1, 55.1, 57.1, 59.1, 61.0, 63.0, 65.0, 66.9, 68.9, 70.9, 72.8,
        #                     74.8, 76.8, 78.7, 80.7, 82.7, 84.6, 86.6, 88.6, 90.6, 92.5, 94.5, 96.5, 98.4, 100.4, 102.4, 104.3, 106.3, 
        ...
        if len(drill_layer):
            for i in range(0, len(drill_layer)): 
                ...
                epcam_api.select_features_by_filter(job, step, [_layer_name])
                ...
                if len(attribute_size):
                    for k in range(0, len(attribute_size)):
                        ...
                        for _user in drill_user_symbols:
                            ...
                            epcam_api.resize_global(job, step, [tem_layer], 1, _size)
                        ...
                        if len(drill_name_list):
                            for t in range(len(drill_name_list)):
                                ...
                                if attribute_name == [{".via_type": "laser"}]:
                                    layer_info.sel_move_other(job, step, [final_layer], job, step, tem_layer, False, 0, 0, 0, 0, 0, 0, 0)
                                    epcam_api.resize_global(job, step, [tem_layer], 1, drill_name_list[t][1])
                                    layer_info.sel_move_other(job, step, [tem_layer], job, step, _layer_name, False, 0, 0, 0, 0, 0, 0, 0)
                                ...
                                if isEnDiameter:
                                    ...
                                    real_size = DrillProcess.get_drill_diameter_size(diameter_list, good_size, attribute_name)
                                ...
                                layer_info.sel_move_other(job, step, [final_layer], job, step, tem_layer, False, 0, 0, 0, 0, 0, 0, 0)
                                epcam_api.resize_global(job, step, [tem_layer], 1, real_delta)
                                #判断孔是否需要添加小孔
                                if len(drill_satellitehole):
                                    for h in range(len(drill_satellitehole)):
                                        if real_size >= drill_satellitehole[h]['drill_min'] and real_size <= drill_satellitehole[h]['drill_max']:                      
                                            #切刀径后的值来进行判断
                                            DrillProcess.replace_drill_by_hole(job, step, tem_layer, real_size, drill_satellitehole[h], attribute_name)
                                layer_info.sel_move_other(job, step, [tem_layer], job, step, _layer_name, False, 0, 0, 0, 0, 0, 0, 0)
                                layer_info.reset_select_filter()
                                ...
                                layer_info.clear_select(job, step, layer_name)                   
                        #处理槽孔
                        DrillProcess.add_new_oval_drill(job, step, layer_name, oval_name_list, attribute_name, oval_size, diameter_list, long_resize_size, short_resize_size, _layer_name, isEnDiameter, oval_range, short_aspect_ratio, ultrashort_slot_resize)
                        DrillProcess.add_new_line_drill(job, step, _layer_name, attribute_name, oval_size, _size, diameter_list, long_resize_size, short_resize_size, isEnDiameter, oval_range, short_aspect_ratio, ultrashort_slot_resize)
                        layer_info.reset_select_filter()
    ...
    #线属性槽孔添加导引孔
    def line_drill_add_pad(self, job, step, paras):
        drill_list = layer_info.get_drill_layer_name(job)   #获取孔层
        aspect_ratio = paras['drill']['aspect_ratio']                  #长短槽比
        drill_shortgroove = paras['drill']['drill_shortgroove']        #短槽引导孔
        isEnDiameter = paras['drill']['IsEnDismeter']
        oval_range = paras['drill']['big_slot']                        #锣槽（槽孔）的范围
            ...
            for _drill in drill_list:
                ...
                if len(line_list):
                    for line_info in line_list:
                        ...
                        diameter_list = DrillProcess.get_diameter_list(isEnDiameter)       #获取刀径表
                        max_size = hole_size
                        real_size = 0
                        if isEnDiameter:
                            #刀径表单位是mil 先转换单位
                            ...
                            hole_size = DrillProcess.get_drill_diameter_size(diameter_list, hole_size, line_attribute)
                            hole_size = hole_size * 25400
                        ...
                        min_size = DrillProcess.get_min_drill(job, step, layer)            #如果孔径小于该层的最小孔径则不添加小孔
    ...
    def add_new_oval_drill(job, step, layer, oval_name_list, attributes, oval_size, diameter_list, long_resize_size, short_resize_size,
        ...
        if len(drill_name_list):
            for i in range(len(drill_name_list)):
                ...
                if isEnDiameter:
                    #刀径表单位是mil 不用转换单位
                    real_size = DrillProcess.get_drill_diameter_size(diameter_list, good_size, attributes)
                else:
                    #刀径表单位是纳米 先转换单位
                    real_size = DrillProcess.get_drill_diameter_size(diameter_list, good_size * 25400, attributes)
        ...
        if len(line_list):
            ...
            for line_info in line_list:
                ...
                if final_dis == 0:                              #线的圆孔
                    ...
                    if isEnDiameter:
                        #刀径表单位是mil 不用转换单位
                        real_size = DrillProcess.get_drill_diameter_size(diameter_list, round(new_line_width / 25400, 3), attributes)
                        new_line_width = real_size      #单位是mil
                    else:
                        #刀径表单位是纳米 先转换单位
                        real_size = DrillProcess.get_drill_diameter_size(diameter_list, new_line_width, attributes)
                ...
                if isEnDiameter:
                    #刀径表单位是mil 不用转换单位
                    real_size = DrillProcess.get_drill_diameter_size(diameter_list, round(new_line_width / 25400, 2), attributes)
    ...
    #锣槽移到外形线层
    def oval2outline(self, job, step, paras):  
        try:
            oval_range = paras['drill']['big_slot']                        #锣槽（槽孔）的范围  
            round_range = paras['drill']['big_drill']                      #锣槽（圆孔）的范围  
            outline_size = paras['outter']['outline_width']
            #获取孔层
            oval_range = oval_range * 25400
            outline_size = outline_size * 25400
            round_range = round_range * 25400
            if oval_range <= 0 or round_range <= 0:
                return
            all_layer_list = []            #所有层名
