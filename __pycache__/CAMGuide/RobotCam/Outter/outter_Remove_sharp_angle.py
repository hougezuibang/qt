...
class Mycal(QThread):
    ...
    def run(self):
        ...
        radius = inner_param['outter']['coverage_min'] * 25400
        radius = radius - 2540
        if radius < 0:
            radius = 2540
        inner_list = []
        inner_list = layer_info.get_outter_list(self.jobname)
        epcam_api.remove_sharp_angle(self.jobname, self.stepname, inner_list, 1, radius, 1, 1)
        time.sleep(1)
        self.finished.emit() 
...
def main(jobname):
    try:
        ...
        b.stepname = step
        b.data = drill_paras
        a = ui_outter_module.processbar_UI(b)
        a.exec_()
