...
class Mycal(QThread):
    ...
    def run(self):
        ...
        执行丝印层扩大线宽
        '''
        silkscreen_list = layer_info.get_silkscreen_layer(self.jobname)
        if len(silkscreen_list)==0:
            return 
        # #删除文字所有属性
        # epcam_api.modify_attributes(job, step, silkscreen_list, 1, [])
        silk_data = self.data
        min_value = silk_data['outter']['outline_width']
        min_name = 'r' + str(min_value)
        min_value = min_value * 25400
        for _layer in silkscreen_list:
            layer_info.reset_select_filter()
            layer_info.set_featuretype_filter(70)
            layer_info.select_features_by_filter(self.jobname, self.stepname, [_layer])
            info_list = layer_info.get_features_infos(self.jobname, self.stepname, _layer)    #获取所有line的信息
            layer_info.reset_select_filter()
            layer_info.clear_select(self.jobname, self.stepname, _layer)  #清除选中并对每一个line进行选中判断 
            #根据线宽来对线进行分类
            name_list = []     #symbolname集合
            name_id_list = {}
            if len(info_list):
                for _line_info in info_list:   
                    if _line_info[2] not in name_list:
                        name_list.append(_line_info[2])
                        name_id = {_line_info[2]:[]}
                        name_id_list.update(name_id)
                    if _line_info[2] in name_id_list:
                        name_id_list[_line_info[2]].append(_line_info[10])
            #遍历name列表，判断并选中
            if len(name_list):
                for _name in name_list:   
                    line_width = layer_info.get_drillsize_by_symbolname(_name)   #通过symbolname获取线宽
                    if line_width < min_value:
                        if _name not in name_id_list:
                            continue
                        id_list = name_id_list[_name]
                        epcam_api.select_feature_by_id(self.jobname, self.stepname, _layer, id_list)
                        layer_info.change_feature_symbols(self.jobname, self.stepname, [_layer], min_name) 
                    layer_info.clear_select(self.jobname, self.stepname, _layer)
            # if len(info_list):
            #     for _line_info in info_list:                       
            #         epcam_api.select_feature_by_id(self.jobname, self.stepname, _layer, [_line_info[10]])
...
def main(jobname):
    try:
        ...
        b.stepname = step
        b.data = drill_paras
        a = ui_silk_module.processbar_UI(b)
        a.exec_()
