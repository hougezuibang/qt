...
from drill_process import DrillProcess
import epcam
import epcam_api
import layer_info
...
class Mycal(QThread):
    ...
    def run(self):
        ...
        执行孔补偿功能
        '''
        # param_str = epcam_api.getParameter()
        # data = json.loads(param_str)
        # data = data['paras']['parameter']
        DrillProcess().drill_resize_to(self.jobname, self.stepname, self.data)
        time.sleep(1)
        self.finished.emit() 
...
def main(jobname):
    try: 
        ...
        if drill_paras == "":
            return
        drill_paras = json.loads(drill_paras, strict = False)
        if 'drill' not in drill_paras:
            return

        #DrillProcess().drill_resize_to(job, step, drill_paras)
...
if __name__ == "__main__":
    ...
    #DrillProcess().drill_resize_to(job, step, data)
    # b = Mycal()
    # b.jobname = job
    # b.stepname = step
    ...
    # xxx = json.dumps(data)
    # epcam_api.setParameter(xxx)
    # bb = epcam_api.getParameter()
    # bb = json.loads(bb)
    # xx = bb['paras']['parameter']
