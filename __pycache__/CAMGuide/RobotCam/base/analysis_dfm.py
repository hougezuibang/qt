...
def drill_check(job, step, layers, erf, rout_distance, hole_size, extra_holes, hole_seperation, power_ground_short, missing_hole, 
    ...
    try:
        ranges_path=os.path.dirname(os.path.dirname(__file__)) + r'\rangesconfig'
...
def solder_mask_DFM(job, step, layers, erf, clearance_min, clearance_opt, coverage_min, coverage_opt, bridge_size, 
    ...
    try:
        ranges_path=os.path.dirname(os.path.dirname(__file__)) + r'\rangesconfig'
        solder_mask_DFM_ranges_path = ranges_path + r'\DFM_SolderMaskOpt\SolderMaskOpt.json'
...
def solder_mask_check(job, step, layers, erf, sm_ar, sm_coverage, sm_to_rout, sliver_min, spacing_min, bridge_min, overlap,
                        ...
                        use_compensated_rout, min_sliver_len, dist2sliver_ratio, max_botteneck_width, apply_range, classify_pad_ar):
    try:
        ranges_path=os.path.dirname(os.path.dirname(__file__)) + r'\rangesconfig'
        solder_mask_check_ranges_path = ranges_path + r'\SolderMaskChecks\SolderMaskRanges.json'
        epcam_log.logger.info(solder_mask_check_ranges_path)
        epcam_log.logger.info(job_operation.transform_range_data(job_operation.load_json(solder_mask_check_ranges_path)))
...
def silk_screen_check(job, step, layers, erf, spacing, sm_clearance, rout_clearance, smd_clearance, line_width, pad_clearance, 
    ...
    try:
        ranges_path=os.path.dirname(os.path.dirname(__file__)) + r'\rangesconfig'
        silk_screen_check_ranges_path = ranges_path + r'\SilkScreenChecks\SilkScreenRanges.json'
        epcam_api.silk_screen_check(job, step, layers, erf, spacing, sm_clearance, rout_clearance, smd_clearance, line_width, pad_clearance, 
                                    copper_coverage, hole_clearance, apply_to, use_compensated_rout, 
                                    job_operation.transform_range_data(job_operation.load_json(silk_screen_check_ranges_path)))   
    except Exception as e:
        print(e)
