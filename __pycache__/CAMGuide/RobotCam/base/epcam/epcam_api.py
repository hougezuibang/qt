import os,sys
import epcam_base as epcam_base
...
def layer_compare_point(jobname1, stepname1, layername1, jobname2, stepname2, layername2, 
                        tolerance = 0.9*25400, isGlobal = True, consider_SR = True, map_layer_resolution = 200*25400):
    data = {
...
def silk_screen_check(job, step, layers, erf, spacing, sm_clearance, rout_clearance, smd_clearance, line_width, pad_clearance, 
                copper_coverage, hole_clearance, apply_to, use_compensated_rout, ranges):
    data = {
        'func': 'SILK_SCREEN_CHECK',
        'paras': [{'job': job}, 
                  {'step': step}, 
                  {'layers': layers},
                  {'erf': erf}, 
...
#get_rest_cu//残铜无孔无开窗                      单位nm²
def get_rest_cu_rate(job,step,layer,or_and,resolution_define,thickness,holes_slots,include_soldermask,include_drill):
    data = {
...
def get_rest_cu(job,step,layer,resolution_define,thickness):
    ...
    #get_rest_cu_with_drill//残铜有孔无开窗                     单位nm²
def get_rest_cu_with_drill(job,step,layer,resolution_define,thickness):
    ...
    #get_cu_solder_drill//残铜有孔有开窗                     单位nm²
...
def fill_profile(job,step,layer,profile_resize,child_profile_resize,avoid_drill_size,fill_by_pattern,symbolname,dx,dy):
    data = {
            'func': 'FILL_PROFILE',
            'paras': {
                        'job': job,
...
def use_pattern_fill_contours(jobname,stepname,layername,symbolname,dx,dy,break_partial,cut_primitive,origin_point,outline,outlinewidth,outline_invert):
    data = {
...
#生成工艺边
def set_panel_margin(job, step, left, right, top, bottom):   
    data = {
            'func': 'SET_PANEL_MARGIN',
            'paras': {
                        'job': job,
                        'step': step,
                        'left':left,
...
# 盘中孔
def get_smd_bga_with_one_drill(job,step,signal_layer):
    data = {
        'func': 'GET_SMD_BGA_WITH_ONE_DRILL',
        'paras': {'job': job, 
                  'step': step, 
...
def solder_mask_check(job, step, layers, erf, sm_ar, sm_coverage, sm_to_rout, sliver_min, spacing_min, bridge_min, overlap,
                        ...
                        use_compensated_rout, min_sliver_len, dist2sliver_ratio, max_botteneck_width, apply_range, classify_pad_ar, ranges):
    data = {
