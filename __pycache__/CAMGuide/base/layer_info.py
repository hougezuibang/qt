...
def reset_select_filter():
    ...
    """
    try:
        epcam_api.set_select_param(0x7F, False, [], -1, -1, [], 0, True)
    except Exception as e:
        print(e)
    return 0
...
def set_featuretype_filter(featuretype):
    ...
    try:
        ...
        epcam_api.set_select_param(featuretype, has_symbol, symbols,dcode, flag,value, pr_value,sele)
    except Exception as e:
        print(e)
    return 0
...
def set_include_symbol_filter(symbol_list):
    ...
    try:
        ...
        dcode = select_param['dcode']
        if symbol_list == '':
            epcam_api.set_select_param(featuretype, False, symbol_list,dcode, flag,value, pr_value,sele)
        else :
            epcam_api.set_select_param(featuretype, True, symbol_list,dcode, flag,value, pr_value,sele)
    ...
    try:
        ...
        if data['paras'] != False:
            for i in range(len(data['paras'])):
               point_symbolname = [data['paras'][i]['X'] * 25400000, data['paras'][i]['Y'] * 25400000, 
                                   data['paras'][i]['symbolname'], data['paras'][i]['feature_index'],
                                   data['paras'][i]['polarity']]
...
def add_oval_pad(job, step, layer, width, height, location_x, location_y, polarity, dcode, orient, attributes, special_angle = 0):
    """ 添指定宽高的槽孔 
    :param     job（str）: 料名 \n
    :param     step（str）: step \n
    :param     layer(str): 层名 \n
    ...
    try:
        ...
        epcam_api.add_pad(job, step, [], layer, symbol, location_x, location_y, polarity, dcode, orient, attributes, special_angle)
    except Exception as e:
        print(e)
    return 0
...
#改变Matrix
def change_layer_matrix(jobname, layer, context,Type,layname, polarity = 'none'):
    """改变layer的context \n
    :param     jobname(str):  料名 \n
    :param     layer(str): 层名 \n
...
def get_curlayer_min_pad_diam(job, step, layer):
    ...
    try:
        min_diam = -1
        r_diam = -1
        epcam_api.set_select_param(0x41, False, [], -1, 1, [{".drill":"via"}], 0, False)
...
def get_power_ground_layer_list(job):
    """ 获取负片层layer_list\n
    #详细描述：  线路层中的power_ground层 \n
    :param     job(str): 料名 \n
    :returns   signal_layer_list(list)： 负片层的layer名列表 \n
    :raises    error: \n
    Usage：    layer_info.get_power_ground_layer_list(job)->['comp','l1','l2','sold']
    """
    try:
        ret = epcam_api.get_matrix(job)
        data = json.loads(ret)
...
def get_feature_rotated_box(job, step, layer, featureID):
    try:
        ret = epcam_api.get_feature_rotated_box(job, step, layer, featureID)
        ret = json.loads(ret)
        if 'result' in ret:
            return ret['result']
        return {}
