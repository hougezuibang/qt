...
def start_cam_flow(job, pre_step, json_str):
    ...
    if len(step_list) == 0 or len(layer_list) == 0:
        print('no step or layer in this job')
        returndata["result"] = False
        return json.dumps(returndata)

    if pre_step not in step_list:
        returndata["result"] = False
        return json.dumps(returndata)
    ...
    if len(drill_layer) == 0:
        print('no drill layer in this job')
        returndata["result"] = False
        return json.dumps(returndata)

    for _drill in drill_layer:
        job_operation.load_layer(job, step, _drill)
