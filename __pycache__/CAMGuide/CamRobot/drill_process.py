...
def drill_resize_to(job, step, attribute_size, drill_show, long_resize_size, short_resize_size, isEnDiameter, drill_satellitehole, 
    ...
    if len(drill_layer):
        for i in range(0, len(drill_layer)): 
            ...
            save_layer = _layer_name + '_back'
            if save_layer in all_layers:
                epcam_api.clear_selected_features(job, step, save_layer)
                layer_info.counter_election(job, step, save_layer)
                layer_info.delete_feature(job, step, save_layer)
                epcam_api.clear_selected_features(job, step, save_layer)
            else:
                job_operation.create_layer(job, save_layer)
            layer_info.sel_copy_other(job, step, [_layer_name], [save_layer], False, 0, 0, 0, 0, 0, 0, 0)
            #临时层
                ...
                for k in range(0, len(attribute_size)):
                    ...
                    if len(pad_list):
                        for x in range(len(pad_list)):
                            ...
                            drillIndex = pad_list[x][3]
                            drillPos = pad_list[x][4]
                            ...
                            if drill_name[0:4] == 'oval' and drill_name not in user_symbol_list:
                                ovalInfos = layer_info.get_feature_rotated_box(job, step, layer_name, drillIndex)
                                oval_id_list.append({"info":ovalInfos, "pos":drillPos})
                    ...
                    if len(drill_name_list):
                        for t in range(len(drill_name_list)):
                            ...
                            epcam_api.change_feature_symbols(job, step, [tem_layer], 'r' + str(real_size), False)
                            #判断孔是否需要添加小孔
                            ...
                            layer_info.clear_select(job, step, layer_name)                   
                    #处理槽孔
                    add_new_oval_drill(job, step, layer_name, oval_id_list, attribute_name, oval_size, diameter_list, long_resize_size, short_resize_size, _layer_name, isEnDiameter, oval_range, short_aspect_ratio, ultrashort_slot_resize)
...
def add_new_oval_drill(job, step, layer, oval_id_list, attributes, oval_size, diameter_list, long_resize_size, short_resize_size,
                        orig_drill_layer, isEnDiameter, oval_range, short_aspect_ratio, ultrashort_slot_resize):  
    #存放feature id信息
    if len(oval_id_list):
        for feature_pos in oval_id_list:
            #获取该oval的信息
            ovalInfos = feature_pos['info']
            featurePos = feature_pos['pos']
            feature_pos = 1
            if featurePos != 'POS':
                feature_pos = 0
            ...
            # #删除选中feature
            # layer_info.clear_select(job, step, layer) 
            # epcam_api.select_feature_by_id(job, step, layer, featureID)
            # #删除原槽孔
            # epcam_api.sel_delete(job, step, [layer])
            ###################
            oval_width = ovalInfos["width"]
            oval_length = ovalInfos["length"]
            oval_angle = ovalInfos["angle"]
            center_x = ovalInfos["center_x"]
            center_y = ovalInfos["center_y"]
            ...
            #             continue
            #原孔径加上补偿值后与刀径比较
            good_size_width = oval_width + oval_size[0] * 25400
            good_size_height = oval_length + oval_size[1] * 25400
            ...
            if isEnDiameter:
                #刀径表单位是mil 不用转换单位
                real_size = get_drill_diameter_size(diameter_list, round(good_size / 25400, 3), attributes)
            else:
                #刀径表单位是纳米 先转换单位
                real_size = get_drill_diameter_size(diameter_list, good_size, attributes)
                real_size = round(real_size / 25400, 3)
            if resize_width:
                new_width_height = [real_size, round(good_size_height / 25400, 3)]
            else:
                new_width_height = [round(good_size_width / 25400, 3), real_size]
            oval_angle = -1 * oval_angle
            #添加槽孔
            new_width = new_width_height[0]
            new_height = new_width_height[1]
            layer_info.add_oval_pad(job, step, orig_drill_layer, new_width, new_height, center_x, center_y, feature_pos, 0, 8, attributes, oval_angle)
            select_poligon = [[center_x - 1, center_y - 1], [center_x - 1, center_y + 1],
            ...
            if oval_long_size > oval_short_size * 2:
                epcam_api.resize_global(job, step, [orig_drill_layer], 0, long_resize_size)
            else:
                if oval_long_size < oval_short_size * short_aspect_ratio:
                    epcam_api.resize_global(job, step, [orig_drill_layer], 0, ultrashort_slot_resize)
                else:
                    epcam_api.resize_global(job, step, [orig_drill_layer], 0, short_resize_size)
